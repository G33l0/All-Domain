...
DB_PATH = "domains.db"
OUTPUT_DIR = "output"
INITIAL_DELAY = 2  # seconds before starting
...
async def fetch_crt(session: aiohttp.ClientSession, limit=200) -> List[str]:
    """Fetch a page of domains from crt.sh."""
...
async def fetch_fallback(session: aiohttp.ClientSession) -> List[str]:
    """Fallback: fetch popular domains from public suffix list."""
    ...
    async def run(self):
        async with aiohttp.ClientSession(headers={'User-Agent': config.user_agent}) as session:
            while self.running:
                ...
                # Sleep in short steps to allow pause/resume
...
class LiveUI:
    ...
    def render(self) -> Layout:
        ...
        if self.awaiting_start:
            status = "⏳ WAITING"
        stats_text = (
            f"{status}  |  Processed: {self.stats['processed']}  |  "
            f"Responsive: {self.stats['responsive']}  |  New: {self.stats['new']}  |  Queue: {self.stats['queue']}"
        )
        ...
        if not sorted_techs:
            table.add_row("(waiting for data)", "0")
        ...
        # Footer
        footer = Panel(
            Align.center("[bold]Commands: start | pause | resume | config | status | exit[/]"),
            border_style="grey50",
            box=box.SIMPLE
        )
    ...
    async def reader(self):
        ...
        while self.running:
            try:
                ...
                if not cmd:
                    break
                await self.cmd_queue.put(cmd.strip())
            except:
                break
...
class DomainCollector:
    ...
    async def run(self):
        ...
        cmd_handler = CommandHandler(self.ui, self.producer, self)
        reader_task = asyncio.create_task(cmd_handler.reader())
        processor_task = asyncio.create_task(cmd_handler.processor())
        # Start producer
        ...
        self.tasks = [ui_task, reader_task, processor_task, producer_task] + consumers

        # Wait for start command or exit
